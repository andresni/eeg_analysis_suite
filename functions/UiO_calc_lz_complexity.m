...
function [C, H, gs] = UiO_calc_lz_complexity(S, type, normalize)
...
while n <= length(S)
    ...
    k = 1;
    while k <= ceil(length(idx_list)/2);
...
disp('calculate terminal points');
...
disp('use terminal points for production sequence');
...
if normalize
    ...
    %n=length(S);%(n / log2(n))
    C = length(H) / (length(S)*ent / log2(length(S)));
else
